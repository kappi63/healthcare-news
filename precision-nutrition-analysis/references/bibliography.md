...
| S033 | 検査値の交絡に関する一般知見：飲酒（γ-GTP・AST>ALT・MCV・TG・尿酸・HDL・フェリチン上昇）／喫煙（Hb・Ht・WBC上昇、HDL低下）／急性期反応（フェリチン偽高・血清鉄低下・亜鉛低下・アルブミン低下）／採血条件（非空腹時TG、コルチゾール日内変動、血清鉄日内変動、脱水・溶血）／高用量ビオチンの免疫測定法干渉／エストロゲンによる銅上昇／クレアチニンの筋肉量依存 | 判定前の交絡チェック | confounders.md / intake-schema.md / output-checklist.md | 確立 | 2026-08 |
| S034 | カンジダ：消化管・口腔の常在真菌であり検出＝異常ではない。抗体は既往曝露でも陽性。不定愁訴を「腸カンジダ過剰増殖」で説明する枠組みは確立した診断体系ではない。／オリゴスキャン：経皮分光によるミネラル・重金属推定。血液・尿の標準法との一致と再現性を示す査読データが乏しい | 解析対象外検査の扱い | functional-tests/pending-tests.md / intake-schema.md | 否定的〜妥当性未確立 | 2026-08 |
| S035 | H. pylori 抗体の解釈：除菌後も抗体価の低下に年単位を要するため陽性＝現感染ではない。高度萎縮では菌量減少により抗体が陰性化しうるため、陰性で萎縮を否定しない。ペプシノーゲン法との併用が前提 | 低胃酸パターンの鑑別 | patterns.md(A1) / terms.json(h-pylori) | 確立 | 2026-08 |
| S036 | 薬機法（医薬品医療機器等法）66条 虚偽・誇大広告の禁止／医療広告ガイドライン／景品表示法。および対面説明における専門用語の伝達失敗に関する運用知見 | 表現ガード・言い換え先出し | expression-guard.md / plain-language.md / output-checklist.md | 法令・運用設計 | 2026-08 |
